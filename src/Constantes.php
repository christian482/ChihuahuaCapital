...
class Constantes {
  ...
  const PEDIR_NUMERO_IMSS = "Escriba su numero de seguro social (En caso de no contar con el escriba: “x”)";
  ...
  const PREGUNTAR_ESTADO = "¿En que estado te encuentras?";
  const PREGUNTAR_DEPENDENCIA = "¿En qué tipo de dependencia trabajas?";
  ...
  const MENSAJE_SOY_JUBILADO_IMSS = "Perfecto, por ser Jubilado O PENSIONADO  tiene derecho a su crédito personalizado ";
  const MENSAJE_SOY_ACTIVO = "Perfecto, por ser ACTIVO tiene derecho a su crédito personalizado";
  const MENSAJE_SOY_NINGUNO = "Otorgamos créditos a otros sectores como CFE Y PEMEX  entro otros. ";
  const MENSAJE_ESCRIBA = "Escriba los datos que le solicitamos y en breve se contactará un agente especializado. ";

  const ADJUNTA_TARJETONES = "Si cuenta con los últimos  tarjetones y/o informes de pago envíe una foto, si no omita este paso. ";
  const ADJUNTA_TALONES = "Si cuenta con los últimos talones de pago envíe una foto, si no omita este paso. ";
  const MENSAJE_DESPEDIDA = "Perfecto. Tiene asignado a su asesor ANA TORRES WhatsApp:6141735841 ";
  const EXPLICAR_SERVICIO ="Credifintech se enfoca en apoyar a los Jubilados , Pensionados Y ACTIVOS  del IMSS,ISSTE Y CFE  con un gran beneficio";
}
